...
df_outcome_participation <- df_both %>% 
  ...
  pivot_wider(
    id_cols = c("response_id","treatment", "treatment_60_64", "treatment_50_59")
    , names_from = wave
...
df_outcome_trips <- df_outcome_trips %>%  dplyr::left_join(
  ...
  , by = c("response_id","treatment", "treatment_60_64", "treatment_50_59")
) %>% 
  relocate(
    c(count_trips_total,avg_trips_daily_total)
    , .after = treatment_50_59
  ) 
...
df_outcomes <- dplyr::left_join(
  df_outcome_participation %>% 
    select(response_id,treatment, treatment_60_64, treatment_50_59 
           ,dplyr::matches("delta"))
  , df_outcome_cost %>% 
