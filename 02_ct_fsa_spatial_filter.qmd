...
montreal_fsa <- montreal_fsa %>% 
  dplyr::mutate(
    age_50_over_tot = age_50_54_tot + age_55_59_tot + age_60_64_tot + age_65_over_tot
    , prop_age_65_over_tot = age_50_over_tot / cd_pop_fsa
  )
```
...
f_under_over <- function(df){
  df <- df %>% 
    ...
    mutate(under_over = prop_survey / prop_age_65_over_tot)
  ...
  df <- df %>% 
    mutate(
      ...
      , "<br>"
      , "("
      , prop_age_65_over_tot %>% round(3)
      , ")"
      )
    , label_survey = as.character(prop_survey %>% round(3))
    , label_census = paste0("(", prop_age_65_over_tot %>% round(3), ")")
    )
}
