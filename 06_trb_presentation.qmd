...
Create new income groups by grouping original ones
...
df_first <- df_both %>% 
  filter(wave=="First") %>% 
  mutate(treatment_50_64 = case_when(
    
    response_id %in% setDT(df_second)[treatment_50_64=="Treatment",response_id] ~ "Treatment"
    
    , response_id %in% setDT(df_second)[treatment_50_64=="Control",response_id] ~ "Control"
...
df_outcome_participation <- df_both %>% 
  select(
    response_id,wave
    ,treatment,treatment_60_64, treatment_50_64
    ,satisfaction_activity_participation
    , q109_3, q109_4, q109_5
    ) %>% 
  pivot_wider(
    id_cols = c("response_id","treatment", "treatment_60_64", "treatment_50_64")
    , names_from = wave
...
df_outcome_trips <- df_outcome_trips %>%  dplyr::left_join(
  ...
  , by = c("response_id","treatment", "treatment_60_64", "treatment_50_64")
) %>% 
  relocate(
    c(count_trips_total,avg_trips_daily_total)
    , .after = treatment_50_64
  ) 
...
df_outcomes <- dplyr::left_join(
  df_outcome_participation %>% 
    select(response_id,treatment, treatment_60_64, treatment_50_64 
           ,dplyr::matches("delta"))
  , df_outcome_cost %>% 
...
ub99 <- quantile(df_outcome_trips$avg_trips_daily_total, 0.99)
