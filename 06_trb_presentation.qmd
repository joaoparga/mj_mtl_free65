...
df_outcome_trips <- df_outcome_trips %>%  dplyr::left_join(
  df_second %>% 
    select(response_id,treatment,count_trips_total)
...
If one needs only 1 leg to work/study + 1 extra activity every day, that would result in 4 trips on a regular/normal day.
On an exceptional day, that count would go up to 1 or 2 additional round trips (6 or 8)
...
```{r}
summary(df_outcome_trips$count_trips_total)

df_outcome_trips %>%
  ggplot(
    aes(x=count_trips_wave_second)
    ) +
...
df_tc %>%
  ggplot(
    aes(y=count_trips_wave, x = treatment, group = treatment)
    ) +
   geom_boxplot(
     # position = "jitter"
     ) +
