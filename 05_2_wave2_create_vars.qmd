...
wave1_original <- sf::st_read("data/report/survey_filter_inside_cd.gpkg")
```
...
<!--   df <- df %>%  -->
<!--     dplyr::left_join(df_translated_v2_q45_21 %>% select(-v2_q45_21_text), by = "v1") %>% -->
<!--     dplyr::left_join(df_translated_v2_q21_14_text %>% select(-v2_q21_14_text), by = "v1") -->
<!-- } -->
...
```{r}
df_wave1 <- df_full %>% select(1:q75_1)
df_wave1 <- df_wave1 %>% 
  mutate(wave = "First") %>% 
  relocate("wave", .after = "v1")

df_wave2 <- df_full %>% select(1, starts_with("v2"))
df_wave2 <- df_wave2 %>% 
  rename_with(~str_remove(., "^v2_")) %>% 
  mutate(wave = "Second") %>% 
  relocate("wave", .after = "v1")
...
df_wave1_only <- df_full %>% 
  select(v1, all_of(cols_wave1only))

df_wave2_only <- df_full %>% 
  select(v1, all_of(cols_wave2only)) %>% 
  rename_with(~str_remove(., "^v2_"))
...
f_join_waves <- function(){
  ...
  # Add other columns specifying only second wave (since 1st is demographics)
  ...
  # add translated
  df_both <- df_both %>% 
  dplyr::left_join(df_trans_bind, by = c("v1", "wave"))
...
df_count <- df_both
df_count <- df_count %>% 
  select(c(v1, wave, q21, q22_1:q94_10,q117_1:q117_10)) %>% 
  mutate(across(!c(v1:q21), ~na_if(., -99L)))
...
df_trips <- df_count %>% 
  select(v1,wave,mode_stm_bus_dest_work:mode_rem_dest_volunteer) %>% 
...
df_count_modes <- df_count %>% select(v1, wave) %>% 
  dplyr::left_join(df_modes, by = c("v1", "wave"))
rm(df_modes)
...
df_count_dest <- df_count %>% select(v1, wave) %>% 
  dplyr::left_join(df_dest, by = c("v1", "wave"))
rm(df_dest)
...
df_main_ind_trip <- df_trips[, .SD[which.max(count)], by = .(v1,wave)]

data.table::setDT(df_trips_dest)
df_main_trip_dest <- df_trips_dest[, .SD[which.max(count_trips_dest)], by = .(v1, wave)] %>% 
  mutate(
    main_dest = case_when(
...
df_main_trip_mode <- df_trips_mode[, .SD[which.max(count_trips_mode)], by = .(v1,wave)] %>% 
  mutate(
    main_mode = case_when(
...
df_main_trip_type <- df_trips_type[, .SD[which.max(count_trips_type)], by = .(v1,wave)] %>% 
  mutate(
    main_type = case_when(
...
```{r joincounttrips}
df_both <- df_both %>% 
  dplyr::left_join(l_trips_wave$df_trips_total, by = c("v1")) %>% 
  dplyr::left_join(l_trips_wave$df_trips_wave, by = c("v1","wave")) %>% 
  dplyr::left_join(l_trips_wave$df_count_modes, by = c("v1","wave")) %>% 
  dplyr::left_join(l_trips_wave$df_count_dest, by = c("v1","wave")) %>%
  dplyr::left_join(l_trips_wave$df_trips_dest_wide, by = c("v1","wave")) %>%
  dplyr::left_join(l_trips_wave$df_trips_type_wide, by = c("v1","wave")) %>%  
  dplyr::left_join(l_trips_wave$df_main_trip_dest %>% select(v1,wave,main_dest)
                   , by = c("v1","wave")) %>% 
  dplyr::left_join(l_trips_wave$df_main_trip_mode %>% select(v1,wave,main_mode)
                   , by = c("v1","wave")) %>% 
  dplyr::left_join(l_trips_wave$df_main_trip_type %>% select(v1,wave,main_type)
                   , by = c("v1","wave"))
```
...
save(l_trips_wave, file = "data/list_count_trips.RData")
```
