...
## LIM data
```{r}
load("data/statscan_lim_threshold/statscan_limthreshold_formatted.RData")
```
...
df_response_rate <- df_full %>% 
  ...
  mutate(response_rate = as_factor(case_when(
    !is.na(v2_start_date) ~ "Started wave 2"
...
df_wave1_only <- df_wave1_only %>% 
  mutate(
    ...
    # age categories
    , age_groups_5_grouped = as_factor(case_when(
    ...
    , age_groups_binary = case_when(
      age_corrected < 50L ~ "age_less_50"
      , age_corrected >= 50L & age_corrected <= 64L ~ "50-64"
      , age_corrected >= 65L ~ "65+" 
      , q40 == "Prefer not to answer" & q4 == "50-64" ~ "50-64"
      , q40 == "Prefer not to answer" & q4 == "65 and older" ~ "65+"
      , .default = NA_character_
    )
...
```{r newincome}
# create variable with separators
df_both <- df_both %>% 
  mutate(
    income_groups = case_when(
      q31== "I prefer not to answer" ~ NA_character_
      , .default = str_trim(q31)
    )
    , income_2 = case_when(
      income_groups == "Less than $15,000" ~ "$0-$14999"
...
## lower
lower <- df_both$income_2 %>% 
  unique() %>% 
  str_extract("\\$[[:digit:]]{1,6}") %>% 
  str_remove("\\$") %>% 
  discard(is.na) %>% 
  as.numeric() %>% 
  sort()
## upper
upper <- df_both$income_2 %>% 
  unique() %>% 
  str_extract("[[:digit:]]{1,6}$") %>% 
  discard(is.na) %>% 
  as.numeric() %>% 
...
# append last value from lower
upper <- c(upper,lower[length(lower)])


# include column with lower and upper end to categorize
vec_income <- df_both$income_2 %>% unique()

df_both <- df_both %>% 
  mutate(
    income_2 = case_when(
      income_2 == "$210000 or more" ~ "$210000-$210000"
      , .default = income_2
    )
  )

# create lower and upper columns
df_both <- df_both %>% 
  mutate(
    lower_range = income_2 %>% 
    str_extract("\\$[[:digit:]]{1,6}") %>% 
    str_remove("\\$") %>% 
    as.numeric()
    
    , upper_range = income_2 %>% 
    str_extract("[[:digit:]]{1,6}$") %>% 
    as.numeric()
  )
...
df_both <- df_both %>% 
  dplyr::left_join(
    df_lim %>% select(household_size,lim_cutoff)
    , by = "household_size"
  )
...
df_both <- df_both %>% 
  mutate(
    income_midpoint = round(
      (lower_range + upper_range) / 2, digits = 0)
    ) %>% 
  relocate(income_midpoint, .before = upper_range)

# classify
df_both <- df_both %>% 
  mutate(
    below_lim_midpoint = case_when(
      # is.na(household_size) ~ NA_character_
      # , is.na(income_2) ~ NA_character_
      
       lower_range > lim_cutoff ~ "No"
      
      , upper_range < lim_cutoff ~ "Yes"      
      
      , lower_range <= lim_cutoff &
        upper_range >= lim_cutoff ~ "midpoint"
      
      # lim_cutoff < lower_range ~ "no"
      # 
      # , lim_cutoff >= lower_range & 
      #   lim_cutoff <= upper_range ~ "midpoint"
      # 
      # , lim_cutoff > lower_range
      , .default = NA_character_
    )
...
df_both <- df_both %>% 
  mutate(
    in_lim = as_factor(case_when(
      ...
      below_lim_midpoint == "midpoint" &
        income_midpoint <= lim_cutoff ~ "Yes"
      
      , below_lim_midpoint == "midpoint" &
        income_midpoint > lim_cutoff ~ "No"
      
      , .default = below_lim_midpoint
      
    ))
  )

# df_both <- df_both %>% 
#   mutate(in_lim = factor(in_lim, levels = c("No", "Yes")))

```


OLD LIM INCORRECT (ONLY DONE VIA MIDPOINT)
...
df_both <- df_both %>% 
  mutate(
    ...
    , transit_barrier_cost_related = as.factor(case_when(
      # only display q25 to those answering no on q100
      q100 == "No" & 
      str_detect(string = q25, pattern = "I can’t afford it|It’s not worth the cost ") ~ "Yes"
      
      , q100 == "No" &
        str_detect(
        string = q25, pattern = "^$|Prefer not to answer"
        ) |
        is.na(q25) ~ NA_character_
      , .default = "No"
    ))
...
df_trips <- df_count %>% 
  ...
  mutate(
    ...
    , type = case_when(
      mode %in% c("metro", "paratransit", "rail", "stm_bus", "rem") ~ "transit"
      , mode %in% c("walk", "own_bike", "service_bike") ~ "active"
      , mode %in% c("own_car", "carsharing", "carpooling") ~ "car"
      , mode %in% c("other_bus", "motorcycle", "taxi") ~ "other"
      , .default = NA_character_
    )
    )
...
df_main_trip_dest <- df_trips_dest[
  df_trips_dest[, .I[which.max(count_trips_dest)], by =.(response_id,wave)]$V1
]

df_main_trip_dest <- df_main_trip_dest %>% 
  mutate(main_dest = case_when(
...
data.table::setDT(df_trips_mode)
df_main_trip_mode <- df_trips_mode[
  df_trips_mode[, .I[which.max(count_trips_mode)], by =.(response_id,wave)]$V1
]
...
df_main_trip_type <- df_trips_type[
  df_trips_type[, .I[which.max(count_trips_type)], by =.(response_id,wave)]$V1
  ]
...
df_both <- df_both %>% 
  ...
  dplyr::left_join(
    df_count %>% 
      select(response_id, wave, contains("last_7"))
    , by = c("response_id","wave")
    )
```
...
lvls_response <- c("Did not start wave 2", "Started wave 2")
```
...
df_both <- df_both %>% 
  mutate(
    ...
    , age_groups_5_grouped = fct_relevel(age_groups_5_grouped, lvls_age_groups_grouped)
...
df_both <- df_both %>% 
  mutate(
    satisfaction_seeing_friends_family = factor(
      satisfaction_seeing_friends_family
      , levels = c("Dissatisfied", "Neither", "Satisfied")
      , ordered = T
    )
    , satisfaction_recreation = factor(
      satisfaction_recreation
      , levels = c("Dissatisfied", "Neither", "Satisfied")
      , ordered = T
    )
    , satisfaction_community_events = factor(
