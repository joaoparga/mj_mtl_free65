...
survey_filter_cd <- survey_filter_cd %>% 
  mutate(
    across(q22_1:q96_10, ~dplyr::na_if(., -99L))
  )
```
...
df_count <- df_count %>% 
  select(v1, q22_1:q94_10)

df_count <- df_count %>% 
  rename_with(.fn = ~str_replace(., "^q22", "mode_stmbus")) %>% 
  rename_with(.fn = ~str_replace(., "^q95", "mode_metro")) %>% 
  rename_with(.fn = ~str_replace(., "^q82", "mode_paratransit")) %>% 
  rename_with(.fn = ~str_replace(., "^q83", "mode_otherbus")) %>% 
  ...
  rename_with(.fn = ~str_replace(., "^q91", "mode_car_sharing")) %>% 
...
# count total trips
df_trips <- df_count %>% 
  # select(v1,stm_bus_work:stm_bus_volunteer) %>% 
  st_drop_geometry() %>% 
  pivot_longer(cols = mode_stmbus_dest_work:mode_motorcycle_dest_volunteer
               ,names_to = "type", values_to = "count") %>% 
  group_by(v1) %>% 
  summarise(count_trips = sum(count, na.rm = T)) 

6666666 CONTINUAR AQUI
# count modes
a <- df_count %>% 
  # select(v1,stm_bus_work:stm_bus_volunteer) %>% 
  st_drop_geometry() %>% 
