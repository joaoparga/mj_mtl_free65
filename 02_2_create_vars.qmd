...
survey_filter_cd <- survey_filter_cd %>% 
  mutate(
    ...
    , when_cost_transport_barrier = as_factor(case_when(
      ...
      , .default = "Previous than last week"
    ))
...
lvls_when <- c("Within last week", "Previous than last week")
...
survey_filter_cd <- survey_filter_cd %>% 
  mutate(
    across(
      c(in_lim, educ_has_postsec, lone_household, employed, unemployed, immigrant
        , any_functional_limitation, any_mobility_limitation
        , firstnations_metis_inuit, transit_once_week_plus, last_7_mode_transit
        , last_7_mode_car, last_7_mode_active, awereness_free_pas65
        , has_free_pass, rides_transit_as_would_like, has_cost_prevented_transit
...
sf::st_write(survey_filter_cd, "data/report/survey_filter_inside_cd.gpkg",
             append = F)
save(l_trips, file = "data/list_count_trips.RData")
```
