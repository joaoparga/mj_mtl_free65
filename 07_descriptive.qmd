...
t_age_control_3 <- df_both %>% 
  filter(treatment_50_64=="Control" & wave =="First") %>%
  select(treatment_50_64, age_groups_5_grouped) %>% 
   
    mutate(
    across(
      .cols = -one_of("treatment_50_64")
      # don't change fct below for newer one
      , ~ forcats::fct_explicit_na(.x)
      )
  ) %>% 
...
tbl_merge(
  ...
  , tab_spanner = c(
    ...
    ,"**Control: 65+ and no pass**", "**Control: 60-64**", "**Control: 50-64**"
    )
)
```
...
t_basic_treatment_3 <- df_both %>% 
  ...
  mutate(
    treatment_50_64 = factor(
      treatment_50_64
      , levels = c("Treatment", "Control")
      , labels = c("Treatment","Control: 50-64")
      )
  ) %>% 
  select(
# controls
...
```{r}
df_both %>% 
  filter(!is.na(treatment_50_64)) %>%
  group_by(age_corrected, wave, treatment_50_64) %>%
  ...
  ggplot(
    aes(
      ...
      , y = avg_trips_age
      , colour = treatment_50_64
      , group = treatment_50_64
      )
  ) + 
  geom_point() +
  geom_smooth() +
...
Average number of modes per age vs distance to threshold
...
df_both %>% 
  filter(!is.na(treatment_50_64)) %>%
  group_by(age_corrected, wave, treatment_50_64) %>%
  summarise(avg_modes_age = mean(count_modes, na.rm = T)) %>% 
  mutate(
    age_distance_threshold = case_when(
      is.na(age_corrected) ~ NA_integer_
      , .default = age_corrected - 65L
...
Average number of `dest` per age vs distance to threshold
...
df_both %>% 
  filter(!is.na(treatment_50_64)) %>%
  group_by(age_corrected, wave, treatment_50_64) %>%
  summarise(avg_dest_age = mean(count_dest, na.rm = T)) %>% 
  mutate(
    age_distance_threshold = case_when(
      is.na(age_corrected) ~ NA_integer_
      , .default = age_corrected - 65L
